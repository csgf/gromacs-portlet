...
export DPNS_HOST=$6
chmod 600 ${PROXY_RFC}
GROMACS_RELEASE=$7 #v4_6_5 or v5_0_4
...
if [ "X${MIDDLEWARE}" = "XgLite" ] ; then
	...
	WELCOME=" [ STARTING GROMACS ] in e-Infrastructures based on the EMI-gLite Middleware"
	
	if [ "X${RELEASE}" == "Xel5" ] ; then
		GROMACS_BIN_PATH=${VO_SWPATH_CONTENT}/gromacs-4.6.5-sl5-x86_64-gccany/bin
	else
		if [ "X${GROMACS_RELEASE}" == "Xv4_6_5" ] ; then
                        GROMACS_BIN_PATH=${VO_SWPATH_CONTENT}/gromacs-4.6.5-sl6-x86_64-gccany/bin
                else
                        GROMACS_BIN_PATH=${VO_SWPATH_CONTENT}/gromacs-5.0.4-sl6-x86_64-gccany/bin
                fi
	fi

	# Check if the GROMACS_BIN_PATH is available in the computing resource
        echo;echo "[ CHECHING the GROMACS_BIN_PATH env. variable ... ]"
        if [ ! -d "${GROMACS_BIN_PATH}" ]; then
                echo "The SW path: ${GROMACS_BIN_PATH} NOT FOUND in this server."
                echo "The release: ${GROMACS_RELEASE} NOT FOUND in this remote cluster."
                echo "Trying to run the simulation with older SW release (v4.6.5)"
		GROMACS_RELEASE="v4_6_5"
                if [ "X${RELEASE}" == "Xel5" ] ; then
                        GROMACS_BIN_PATH=${VO_SWPATH_CONTENT}/gromacs-4.6.5-sl5-x86_64-gccany/bin
                else
                        GROMACS_BIN_PATH=${VO_SWPATH_CONTENT}/gromacs-4.6.5-sl6-x86_64-gccany/bin
	...
	echo "GROMACS_RELEASE   : "${GROMACS_RELEASE}
	echo "-----------------------------------------"
...
if [ -f ${GROMACS_BIN_PATH}/mdrun ] ; then
	echo;echo "[ STARTING GROMACS with input file = "$INPUT_FILE " ]"
