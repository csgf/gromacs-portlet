...
if [ "X${MIDDLEWARE}" = "XgLite" ] ; then
	...
	WELCOME=" [ STARTING GROMACS-5.0.4 ] in e-Infrastructures based on the EMI-gLite Middleware"
	
	if [ "X${RELEASE}" == "Xel5" ] ; then
		GROMACS_BIN_PATH=${VO_SWPATH_CONTENT}/gromacs-5.0.4-sl5-x86_64-gccany/bin
	else
...
if [ -f ${GROMACS_BIN_PATH}/mdrun ] ; then
	echo;echo "[ STARTING GROMACS-5.0.4 with input file = "$INPUT_FILE " ]"
